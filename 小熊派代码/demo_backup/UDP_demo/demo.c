...
#define CONFIG_SERVER_IP            "192.168.107.74"                     // 要连接的服务器IP
...
static void uart_read_int_handler(const void *buffer, uint16_t length, bool error)
{
    ...
    // 直接通过Socket发送数据
    sendto(my_g_sock_fd, (uint8_t *)buff, length, 0, (struct sockaddr *)&my_g_send_addr, my_g_addr_length);
}
...
static void *sle_uart_client_task(const char *arg)
{
    ...
    /* 创建Socket */
    if ((my_g_sock_fd = socket(AF_INET, SOCK_DGRAM, 0)) == -1) {
    ...
    my_g_addr_length = sizeof(my_g_send_addr);

    while (1) {
        // 接收服务端返回的数据并通过UART发送
        char recvBuf[512];
        bzero(recvBuf, sizeof(recvBuf));
        recvfrom(my_g_sock_fd, recvBuf, sizeof(recvBuf), 0, (struct sockaddr *)&my_g_send_addr, &my_g_addr_length);
