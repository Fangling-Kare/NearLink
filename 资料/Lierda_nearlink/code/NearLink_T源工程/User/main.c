...
void extract_data(const char *input, char *output) {
    const char *start = strchr(input, '+'); // 查找第一个+号
    if (start == NULL) {
        output[0] = '\0'; // 输入字符串格式不正确，返回空字符串
        return;
    }
    ...
    start = strchr(start, ','); // 查找接收到的数据里面的第一个逗号
    if (start == NULL) {
        output[0] = '\0'; // 输入字符串格式不正确，返回空字符串
        return;
    }
    start++; // 跳过接收到的数据里面的第一个逗号

    start = strchr(start, ','); // 查找接收到的数据里面的第二个逗号
    if (start == NULL) {
        output[0] = '\0'; // 输入字符串格式不正确，返回空字符串
        return;
    }
    start++; // 跳过接收到的数据里面的第二个逗号
...
int main(void)
{
    ...
    //STEP5:发送AT+SEEK=0等待星闪返回OK
    NearLink_SendData(USART2, "AT+SEEK=0\r\n");
    Delay_Ms(100);
    while(strstr((char *)RxBuffer, "OK") == NULL){}

    //STEP6:连接另一块星闪，等待返回OK
